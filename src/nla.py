...
class MindsNLAClient(MindsAPIClient):
    ...
    def get_models(self, verbose=False):
        ''' retrieve available NLA models from server.
        '''
        data = {
            'cmd': 'getNLAModels',
            'ID': self.account_id,
        ...
        with requests.post(
            ...
            if status == NLA_STATUS_OK:
                nla_models = json.loads(payload.get('data'))
            else:
                nla_models = payloads.get('data')
            if verbose: sys.stdout.write('%s\n' % nla_models)
            return (nla_models, status)
